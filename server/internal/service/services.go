...
type transactionRepository interface {
	Create(context.Context, *model.Transaction) error
	GetPaginated(context.Context, uuid.UUID, int, int) ([]model.Transaction, int64, error)
	Delete(context.Context, uuid.UUID, uuid.UUID) error
	GetStatistics(context.Context, uuid.UUID, int, int) (*model.StatisticsData, error)
	GetCalendarData(context.Context, uuid.UUID, int, int) ([]model.CalendarDayData, error)
	CheckOperationExists(context.Context, uuid.UUID) (bool, error)
}

type ledgerMembershipRepository interface {
	IsMember(context.Context, uuid.UUID, uuid.UUID) (bool, string, error)
}
