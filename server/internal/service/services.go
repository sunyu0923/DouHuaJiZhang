...
func (s *LedgerService) IsMember(ctx context.Context, ledgerID, userID uuid.UUID) (bool, error) {
...
func (s *TransactionService) CreateTransaction(ctx context.Context, ledgerID, userID uuid.UUID, req *model.CreateTransactionRequest) (*model.Transaction, error) {
	...
	// Idempotency is checked only after ledger access is confirmed.
...
func (s *TransactionService) DeleteTransaction(ctx context.Context, ledgerID, txID, userID uuid.UUID) error {
	if err := s.requireLedgerMember(ctx, ledgerID, userID); err != nil {
		return err
	}
