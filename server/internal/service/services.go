...
func (s *LedgerService) RequireMember(ctx context.Context, ledgerID, userID uuid.UUID) error {
...
func (s *TransactionService) CreateTransaction(ctx context.Context, ledgerID, userID uuid.UUID, req *model.CreateTransactionRequest) (*model.Transaction, error) {
	...
	// Check idempotency after authorization so operation IDs do not leak across ledgers.
...
func (s *TransactionService) DeleteTransaction(ctx context.Context, ledgerID, userID, id uuid.UUID) error {
	...
	if err := s.txRepo.DeleteFromLedger(ctx, id, ledgerID); err != nil {
		if errors.Is(err, repository.ErrNoRowsAffected) {
			return ErrNotFound
		}
		return err
...
func (s *InvestmentService) DeleteInvestment(ctx context.Context, id, userID uuid.UUID) error {
	if err := s.investmentRepo.Delete(ctx, id, userID); err != nil {
		if errors.Is(err, repository.ErrNoRowsAffected) {
			return ErrNotFound
		}
		return err
...
func (s *HealthService) DeletePoopRecord(ctx context.Context, id, userID uuid.UUID) error {
	if err := s.healthRepo.DeletePoopRecord(ctx, id, userID); err != nil {
		if errors.Is(err, repository.ErrNoRowsAffected) {
			return ErrNotFound
		}
		return err
...
func (s *HealthService) DeleteMenstrualRecord(ctx context.Context, id, userID uuid.UUID) error {
	if err := s.healthRepo.DeleteMenstrualRecord(ctx, id, userID); err != nil {
		if errors.Is(err, repository.ErrNoRowsAffected) {
			return ErrNotFound
		}
		return err
