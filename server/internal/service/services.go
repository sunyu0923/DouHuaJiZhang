...
func (s *TransactionService) CreateTransaction(ctx context.Context, ledgerID, userID uuid.UUID, req *model.CreateTransactionRequest) (*model.Transaction, error) {
	...
	// Idempotency is enforced after authorization so operation IDs cannot be probed across ledgers.
...
func (s *TransactionService) DeleteTransaction(ctx context.Context, ledgerID, id, userID uuid.UUID) error {
	...
	if err := s.requireLedgerMember(ctx, tx.LedgerID, userID); err != nil {
		return err
	}
...
func (s *InvestmentService) DeleteInvestment(ctx context.Context, id, userID uuid.UUID) error {
	deleted, err := s.investmentRepo.DeleteByUserID(ctx, id, userID)
	if err != nil {
		return err
	}
	if !deleted {
...
func (s *HealthService) DeletePoopRecord(ctx context.Context, id, userID uuid.UUID) error {
	deleted, err := s.healthRepo.DeletePoopRecordByUserID(ctx, id, userID)
	if err != nil {
		return err
	}
	if !deleted {
...
func (s *HealthService) DeleteMenstrualRecord(ctx context.Context, id, userID uuid.UUID) error {
	deleted, err := s.healthRepo.DeleteMenstrualRecordByUserID(ctx, id, userID)
	if err != nil {
		return err
	}
	if !deleted {
