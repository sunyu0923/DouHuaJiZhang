...
type transactionService interface {
	GetTransactions(ctx context.Context, ledgerID, userID uuid.UUID, page, pageSize int) ([]model.Transaction, int64, error)
	CreateTransaction(ctx context.Context, ledgerID, userID uuid.UUID, req *model.CreateTransactionRequest) (*model.Transaction, error)
	DeleteTransaction(ctx context.Context, ledgerID, userID, id uuid.UUID) error
	GetStatistics(ctx context.Context, ledgerID, userID uuid.UUID, month, year int) (*model.StatisticsData, error)
	GetCalendar(ctx context.Context, ledgerID, userID uuid.UUID, month, year int) ([]model.CalendarDayData, error)
}
...
func RegisterTransactionRoutes(r *gin.RouterGroup, svc transactionService) {
