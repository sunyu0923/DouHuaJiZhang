...
func RegisterTransactionRoutes(r *gin.RouterGroup, svc *service.TransactionService) {
	r.GET("/:id/transactions", func(c *gin.Context) {
		...
		txns, total, err := svc.GetTransactionsForUser(c.Request.Context(), ledgerID, userID, page, pageSize)
		if err != nil {
	...
	r.DELETE("/:id/transactions/:txId", func(c *gin.Context) {
		...
		if err := svc.DeleteTransaction(c.Request.Context(), ledgerID, txID, userID); err != nil {
			if err == service.ErrForbidden {
				c.JSON(http.StatusForbidden, model.Error(403, err.Error()))
				return
			}
	...
	r.GET("/:id/statistics", func(c *gin.Context) {
		...
		stats, err := svc.GetStatisticsForUser(c.Request.Context(), ledgerID, userID, month, year)
		if err != nil {
			if err == service.ErrForbidden {
				c.JSON(http.StatusForbidden, model.Error(403, err.Error()))
				return
	...
	r.GET("/:id/calendar", func(c *gin.Context) {
		...
		month, _ := strconv.Atoi(c.Query("month"))
		year, _ := strconv.Atoi(c.Query("year"))
		data, err := svc.GetCalendarForUser(c.Request.Context(), ledgerID, userID, month, year)
		if err != nil {
			if err == service.ErrForbidden {
				c.JSON(http.StatusForbidden, model.Error(403, err.Error()))
				return
...
func RegisterInvestmentRoutes(r *gin.RouterGroup, svc *service.InvestmentService) {
	...
	r.DELETE("/:id", func(c *gin.Context) {
		...
		if err := svc.DeleteInvestment(c.Request.Context(), userID, id); err != nil {
			if err == service.ErrNotFound {
				c.JSON(http.StatusNotFound, model.Error(404, err.Error()))
				return
			}
...
func RegisterHealthRoutes(r *gin.RouterGroup, svc *service.HealthService) {
	...
	r.DELETE("/poop/:id", func(c *gin.Context) {
		...
		if err := svc.DeletePoopRecord(c.Request.Context(), userID, id); err != nil {
			if err == service.ErrNotFound {
				c.JSON(http.StatusNotFound, model.Error(404, err.Error()))
				return
			}
	...
	r.DELETE("/menstrual/:id", func(c *gin.Context) {
		...
		if err := svc.DeleteMenstrualRecord(c.Request.Context(), userID, id); err != nil {
			if err == service.ErrNotFound {
				c.JSON(http.StatusNotFound, model.Error(404, err.Error()))
				return
			}
