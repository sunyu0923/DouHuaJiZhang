...
func (r *InvestmentRepository) Delete(ctx context.Context, userID, id uuid.UUID) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM investments WHERE user_id = $1 AND id = $2`, userID, id)
	return err
}
...
func (r *HealthRepository) DeletePoopRecord(ctx context.Context, userID, id uuid.UUID) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM poop_records WHERE user_id = $1 AND id = $2`, userID, id)
	return err
}
...
func (r *HealthRepository) DeleteMenstrualRecord(ctx context.Context, userID, id uuid.UUID) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM menstrual_records WHERE user_id = $1 AND id = $2`, userID, id)
	return err
}
